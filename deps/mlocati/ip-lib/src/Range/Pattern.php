...
namespace ScobyAnalyticsDeps\IPLib\Range;

use ScobyAnalyticsDeps\IPLib\Address\AddressInterface;
use ScobyAnalyticsDeps\IPLib\Address\IPv4;
use ScobyAnalyticsDeps\IPLib\Address\IPv6;
use ScobyAnalyticsDeps\IPLib\Address\Type as AddressType;
use ScobyAnalyticsDeps\IPLib\ParseStringFlag;
/**
...
class Pattern extends AbstractRange
{
    ...
    public function toString($long = \false)
    {
        ...
        switch (\true) {
            case $this->fromAddress instanceof \ScobyAnalyticsDeps\IPLib\Address\IPv4:
                $chunks = \explode('.', $this->fromAddress->toString());
                $chunks = \array_slice($chunks, 0, -$this->asterisksCount);
                $chunks = \array_pad($chunks, 4, '*');
                $result = \implode('.', $chunks);
                break;
            case $this->fromAddress instanceof \ScobyAnalyticsDeps\IPLib\Address\IPv6:
                if ($long) {
