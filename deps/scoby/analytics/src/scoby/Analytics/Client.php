...
class Client
{
     ...
     * @var array
     */
    private array $options = ['generateVisitorId' => \true, 'ipBlackList' => [], 'whitelistedQueryParameters' => ['utm_medium', 'utm_source', 'utm_campaign', 'utm_content', 'utm_term']];
    /**
     * @var LoggerInterface
     */
    ...
    {
        $this->options['ipBlackList'][] = \ScobyAnalyticsDeps\IPLib\Factory::parseRangeString($range);
        return $this;
    }
    /**
    ...
    private function isBlockedIp() : bool
    {
        try {
            if (!empty($this->options['ipBlackList'])) {
                $address = \ScobyAnalyticsDeps\IPLib\Factory::parseAddressString($this->ipAddress);
            ...
            if ($this->logger) {
                $this->logger->warning("scoby - IP blacklist could not be applied: " . $e->getMessage());
            }
        }
        return \false;
    }
    ...
    {
        $urlObj = \ScobyAnalyticsDeps\Spatie\Url\Url::fromString($this->requestedUrl);
    ...
    {
        $urlObj = \ScobyAnalyticsDeps\Spatie\Url\Url::fromString($this->referringUrl);
        $scheme = $urlObj->getScheme();
        $host = $urlObj->getHost();
        return (string) \ScobyAnalyticsDeps\Spatie\Url\Url::create()->withScheme($scheme)->withHost($host);
    }
    /**
    ...
    public function getUrl() : string
    {
        ...
        $params = ["vid" => $this->visitorId, "url" => $this->getRequestedUrl(), "ua" => $this->userAgent];
    ...
    public function logPageView() : bool
    {
        try {
            if ($this->isBlockedIp()) {
                if ($this->logger) {
                    $this->logger->info("scoby - skipped logging page view for blocked IP address.");
                }
                return \false;
            }
            $url = $this->getUrl();
            ...
            if ($statusCode === 204) {
                if ($this->logger) {
                    $this->logger->info("scoby - successfully logged page view (" . $statusCode . "): " . $url);
                }
                return \true;
            } else {
                if ($this->logger) {
                    $this->logger->error("scoby - failed logging page view (" . $statusCode . "): " . $url);
                }
            }
        } catch (Exception|ClientExceptionInterface $exception) {
            if ($this->logger) {
                $this->logger->error("scoby - failed logging page view: " . $exception->getMessage());
            }
        }
        return \false;
    }
