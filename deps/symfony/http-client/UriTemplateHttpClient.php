...
class UriTemplateHttpClient implements HttpClientInterface, ResetInterface
{
    ...
    {
        if (\class_exists(\ScobyAnalyticsDeps\GuzzleHttp\UriTemplate\UriTemplate::class)) {
            return \ScobyAnalyticsDeps\GuzzleHttp\UriTemplate\UriTemplate::expand(...);
        }
        if (\class_exists(\ScobyAnalyticsDeps\League\Uri\UriTemplate::class)) {
            return static fn(string $url, array $vars): string => (new \ScobyAnalyticsDeps\League\Uri\UriTemplate($url))->expand($vars);
        }
        if (\class_exists(\ScobyAnalyticsDeps\Rize\UriTemplate::class)) {
            return (new \ScobyAnalyticsDeps\Rize\UriTemplate())->expand(...);
        }
